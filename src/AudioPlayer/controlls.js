...
export const TogglePlay = () => (
  <Consumer>{({ paused }) => (paused ? <Play /> : <Pause />)}</Consumer>
)
...
export const Mute = () => (
  <Consumer>
    {({ muted, toggleMute }) => {
      ...
      return (
        <Component onClick={toggleMute}>{muted ? 'Unute' : 'Mute'}</Component>
      )
    }}
  </Consumer>
)
...
export const Restart = () => (
  <Consumer>
    {({ ended, restart }) =>
      ended && <Icon.Replay onClick={restart}>Restart</Icon.Replay>
    }
  </Consumer>
)
