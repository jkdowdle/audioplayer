...
export default class AudioPlayer extends Component {
  ...
  determinePercent = (current, duration = this.state.duration) =>
  ...
  handleClickProgress = event => {
    ...
    const { offsetWidth } = this.progressContainer
  ...
  setProgressContainerRef = node => (this.progressContainer = node)

  toggleShelf = () =>
    this.setState(({ displayShelf }) => ({ displayShelf: !displayShelf }))
  ...
  goBack = () => {
    ...
    this.skipToTime(
      backInterval ? backInterval + time : skipInterval * -1 + time
    )
  }
  ...
  goForward = () => {
    ...
    this.skipToTime(
      forwardInterval ? forwardInterval + time : skipInterval + time
    )
  }
  ...
  handleRateChange = ({ target: { playbackRate } }) =>
